...
function tmpmela_setup() {
	...
	* replace to change 'shopinia' to the name of your theme in all
	* template files.
	*/
	load_theme_textdomain( 'shopinia', get_template_directory() . '/languages' );
	/*
	...
	register_nav_menus( array(
		'primary'   => esc_html__( 'TM Header Navigation', 'shopinia' ),
		'header-menu'   => esc_html__( 'TM Header Top Links', 'shopinia' ),
		'topbar-menu'   => esc_html__( 'TM TopBar Links', 'shopinia' ),
		'footer-menu'   => esc_html__( 'Footer Menu Links', 'shopinia' ),
	) );
	/*
...
function tmpmela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'shopinia' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'shopinia' ),
...
function tmpmela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'shopinia' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'shopinia' );
...
function tmpmela_scripts_styles() {
	...
	wp_enqueue_script( 'anime-js', get_template_directory_uri() . '/js/megnor/anime.min.js', array(), '', false);	
}
...
<div class="contributor">
  <div class="contributor-info">
    ...
    <div class="contributor-summary">
      ...
      <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'shopinia' ), $post_count ); ?> </a> </div>
    <!-- .contributor-summary -->
...
<div class="related_posts_outer"><div class="shortcode-title left"><h1 class="normal-title"><?php echo esc_attr_e( 'Related Posts', 'shopinia' ) ?></h1></div>
				...
				<div class="entry-meta-inner">
				  <div class="entry-meta">
					  <div class="entry-summary">
							<div class="excerpt"> <?php echo tmpmela_excerpt(20); ?> </div>
					  </div><!-- .entry-summary -->
					</div> <!--  entry-meta -->
...
return array(
 array(
    'import_file_name'            => 'shopinia',
    'local_import_file'           => trailingslashit( get_stylesheet_directory() ) . 'one-click-installtion/default/shopinia.wordpress.xml',
    'local_import_customizer_file'=> trailingslashit( get_stylesheet_directory() ) . 'one-click-installtion/default/shopinia_customizer_export.dat',
	'local_import_widget_file'    => trailingslashit( get_stylesheet_directory() ) . 'one-click-installtion/default/shopinia_widgets_settings.wie',
    'import_notice'               => esc_html__( 'Please waiting for a few minutes, do not close the window or refresh the page until the data is imported.', 'shopinia' ),
),
);
}
...
function tmpmela_sampledata_after_import($selected_import) {
        ...
        $header_menu = get_term_by('name', 'Header Top Links', 'nav_menu');
		$topbar_menu = get_term_by('name', 'TopBar Menu Links', 'nav_menu');
		$footer_menu = get_term_by('name', 'Footer Menu Links', 'nav_menu');
       ...
       if ( class_exists( 'RevSlider' ) ) {
           $slider_array = array(
              get_template_directory()."/one-click-installtion/default/tmpmela_homeslider.zip",
              get_template_directory()."/one-click-installtion/default/tmpmela_video.zip",
			  );
           ...
           echo esc_html__( 'Slider processed', 'shopinia' );
      }
}
...
/* Whislist Counter */
function tmpmela_update_wishlist_count(){
    if( function_exists( 'YITH_WCWL' ) ){
        wp_send_json( YITH_WCWL()->count_products() );
    }
}
add_action( 'wp_ajax_update_wishlist_count', 'tmpmela_update_wishlist_count' );
add_action( 'wp_ajax_nopriv_update_wishlist_count', 'tmpmela_update_wishlist_count' );

// Order Cancel Button
add_filter( 'woocommerce_valid_order_statuses_for_cancel', 'tmpmela_valid_order_statuses_for_cancel', 10, 2 );
function tmpmela_valid_order_statuses_for_cancel( $statuses, $order ){
